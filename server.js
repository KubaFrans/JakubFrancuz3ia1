...
var tab = [
    { id: 1, login: "Ania", password: "Haslo123", age: 10, uczen: "on", plec: "K" },
    { id: 2, login: "Maciek", password: "zaq1@WSX", age: 17, uczen: "", plec: "M" },
    { id: 3, login: "kretywny_login", password: "kreatywne_haslo", age: 25, uczen: "on", plec: "M" },
]
...
app.post("/register", function (req, res) {
    ...
    for (i = 0; i < tab.length; i++) {
        if (tab[i].login == req.body.login) {
            res.send("Taki uzytkownik istnieje")
            register = false
            break
        } else {
            register = true
...
app.post("/sort", function (req, res) {
    if (req.body.sort == "R") {
        ...
        let tabela = "<pre><a href=sort> sort </a><a href=gender> gender </a><a href=show> show </a></pre><br><form action=/sort method = POST onchange = this.submit()><input type=radio name=sort value=R checked>rosnąco<br><input type=radio name=sort value=M>malejąco<br><table style=border:solid 1px black>"
        for (i = 0; i < tab.length; i++) {
        ...
        let tabela = "<pre><a href=sort> sort </a><a href=gender> gender </a><a href=show> show </a></pre><br><form action=/sort method = POST onchange = this.submit()><input type=radio name=sort value=R>rosnąco<br><input type=radio name=sort value=M checked>malejąco<br><table style=border:solid 1px black>"
        for (i = 0; i < tab.length; i++) {
            tabela += "<tr style=border:solid 1px black><td style=border:solid 1px black> id: " + tab[i].id + "</td><td style=border:solid 1px black>user: " + tab[i].login + "-  " + tab[i].password + "</td><td style=border:solid 1px black> uczeń: "
            if (tab[i].uczen == "on") {
                tabela += "<input type=checkbox checked disabled> </td>"
...
app.get("/:id", function (req, res) {
    switch (req.params.id) {
        ...
        case "sort":
            if (login) {
                let tabela_x = "<pre><a href=sort> sort </a><a href=gender> gender </a><a href=show> show </a></pre><br><form action=/sort method = POST onchange = this.submit()><input type=radio name=sort value=R>rosnąco<br><input type=radio name=sort value=M>malejąco<br><table style=border:solid 1px black>"
                for (i = 0; i < tab.length; i++) {
